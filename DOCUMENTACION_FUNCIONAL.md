...
2. [Conexión y Modos de Acceso](#2-conexión-y-modos-de-acceso)
   ...
   - [2.2. Modal de Configuración y Diagnóstico OAuth](#22-modal-de-configuración-y-diagnóstico-oauth)
   - [2.3. Diagnóstico Automático del Error 403 (Drive API Deshabilitada)](#23-diagnóstico-automático-del-error-403-drive-api-deshabilitada)
   - [2.4. Modo Demostración (Datos de Prueba)](#24-modo-demostración-datos-de-prueba)
   - [2.5. Caché Local y Persistencia (IndexedDB)](#25-caché-local-y-persistencia-indexeddb)
...
7. [Pie de Página y Enlaces Legales/Comunitarios](#7-pie-de-página-y-enlaces-legalescomunitarios)
   - [7.1. Política de Privacidad y Términos de Servicio](#71-política-de-privacidad-y-términos-de-servicio)
   - [7.2. Revocación de Permisos de Google](#72-revocación-de-permisos-de-google)
   - [7.3. Enlace al Repositorio de GitHub](#73-enlace-al-repositorio-de-github)
8. [Controles Generales de la Interfaz](#8-controles-generales-de-la-interfaz)
...
- Capacidad de liberar espacio de forma inmediata y segura (a la papelera o eliminación definitiva).
- Privacidad total: la indexación y el procesamiento se realizan exclusivamente en el navegador del usuario.
...
### 2.2. Modal de Configuración y Diagnóstico OAuth
- **Propósito:** Permite ingresar un **ID de Cliente OAuth personalizado** directamente desde la interfaz sin necesidad de recompilar la aplicación.
- **Funcionalidades del Modal:**
  - Guía interactiva paso a paso con los orígenes autorizados exactos de la URL actual.
  - Detección de errores habituales de configuración (como el error `origin_mismatch`).
  - Botón de copia rápida de la URL de origen autorizada.

### 2.3. Diagnóstico Automático del Error 403 (Drive API Deshabilitada)
- Cuando un proyecto de Google Cloud nuevo aún no tiene habilitada la API de Drive, Google devuelve un error HTTP 403 (`SERVICE_DISABLED` / `accessNotConfigured`).
- La aplicación detecta automáticamente esta respuesta y muestra:
  - Un mensaje descriptivo y amigable en español.
  - Un botón directo: **«Habilitar Drive API en Google Cloud»** que lleva al usuario directamente a la pantalla de activación de su proyecto.

### 2.4. Modo Demostración (Datos de Prueba)
...
### 2.5. Caché Local y Persistencia (IndexedDB)
...
## 7. Pie de Página y Enlaces Legales/Comunitarios

En la parte inferior de la aplicación se encuentra el pie de página permanente con accesos esenciales:

### 7.1. Política de Privacidad y Términos de Servicio
- Modales interactivos integrados que explican de manera transparente el tratamiento de datos (*Client-side Only*), la ausencia de almacenamiento en servidores de terceros y las directrices de seguridad.

### 7.2. Revocación de Permisos de Google
- Enlace directo a `https://myaccount.google.com/permissions` para que el usuario pueda auditar o revocar el acceso de la aplicación en su cuenta de Google en cualquier momento.

### 7.3. Enlace al Repositorio de GitHub
- Enlace directo al repositorio público oficial en GitHub (`https://github.com/pablodeluxe/google-drive-storage-analyzer`) con el ícono de GitHub, permitiendo consultar el código fuente abierto, reportar incidencias o contribuir al proyecto.

---

## 8. Controles Generales de la Interfaz
